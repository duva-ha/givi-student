...
function App() {
    ...
    if (!user) return (
        <div className="h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-900 to-blue-900 text-white p-6 text-center font-bold">
            <div className="mb-8 text-blue-400 text-5xl font-black italic uppercase tracking-tighter drop-shadow-lg">E-Tech Hub</div>
        ...
        <div className="flex h-screen overflow-hidden bg-[#f8fafc]">
                ...
                <nav className="flex-1 space-y-3">
                    {[
                        ...
                        <button key={t.id} onClick={() => { setTab(t.id); setIsFocus(false); }} 
                            className={`w-full flex items-center gap-4 px-6 py-4 text-[11px] font-black uppercase tracking-widest rounded-2xl transition-all ${tab === t.id ? `bg-${t.color}-600 text-white shadow-lg shadow-${t.color}-200 scale-105` : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}>
            ...
            <main className="flex-1 flex flex-col relative overflow-hidden">
                <header className="h-16 px-8 border-b bg-white/50 backdrop-blur-xl flex items-center justify-between z-10">
                    ...
                    <button onClick={() => setIsFocus(!isFocus)} className="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 transition-colors shadow-sm">
                        {isFocus ? '✕' : '⛶'}
                    </button>
                </header>
                ...
                <div className="flex-1 overflow-hidden flex bg-white">
                    {tab === 'baigiang' && (
                        <React.Fragment>
                            <div className={`w-80 bg-slate-50/50 border-r border-slate-100 p-4 overflow-y-auto custom-scroll ${isFocus ? 'hidden' : 'block'}`}>
                        ...
                        <div className="flex-1 p-12 bg-slate-50 overflow-y-auto">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
                                {localQuizzes[grade].map((item, idx) => (
                                    <div key={idx} className="bg-white p-10 rounded-[3.5rem] shadow-xl shadow-slate-200/50 border border-white hover:border-orange-400 transition-all group relative overflow-hidden">
                                        ...
                                        <h3 className="font-black text-slate-700 mb-8 uppercase text-xs tracking-wider leading-relaxed">Bộ câu hỏi Luyện tập Bài {item.quizIndex}</h3>
                                        <button onClick={() => { setActiveQuiz(item.questions); setQuizState({currentQ:0, answers: new Array(item.questions.length).fill(null), showResult:false, reviewMode:false}); setTimeLeft(15 * 60); }} 
                                            className="w-full bg-slate-900 text-white py-5 rounded-[1.8rem] font-black uppercase text-[10px] tracking-widest hover:bg-orange-500 shadow-lg transform active:scale-95 transition-all">Bắt đầu ngay</button>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {tab === 'kiemtra' && (
                        <div className="flex-1 p-12 bg-slate-900 overflow-y-auto flex flex-col items-center">
                            <div className="mb-16 text-center">
                                ...
                                <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.3em]">Hệ thống kiểm tra chất lượng Khối {grade}</p>
                            </div>
                                ...
                                {['HK1', 'HK2'].map(hk => {
                                    ...
                                    return (
                                        <div key={hk} className={`p-12 rounded-[4.5rem] border-2 transition-all text-center backdrop-blur-2xl ${examData ? 'bg-white/5 border-white/10 hover:border-purple-500 shadow-2xl' : 'opacity-20 border-white/5'}`}>
                                            ...
                                            <p className="text-slate-400 mb-10 text-[10px] font-bold uppercase tracking-widest">Thời gian: 45 Phút • Đề chính thức</p>
                                            {examData ? (
                                                <button onClick={() => { setActiveQuiz(examData); setQuizState({currentQ:0, answers: new Array(examData.length).fill(null), showResult:false, reviewMode:false}); setTimeLeft(45 * 60); }} 
                                                    className={`w-full ${hk === 'HK1' ? 'bg-blue-600 shadow-blue-500/20' : 'bg-purple-600 shadow-purple-500/20'} text-white py-6 rounded-[2rem] font-black uppercase text-[11px] tracking-widest shadow-2xl hover:scale-105 transition-all`}>Bắt đầu làm bài</button>
                ...
                {/* MODAL LÀM BÀI / XEM LẠI - CỰC KỲ XỊN XÒ */}
                {activeQuiz && (
                    <div className="fixed inset-0 bg-slate-900/98 backdrop-blur-2xl z-[100] flex items-center justify-center p-4">
                        <div className="bg-white w-full max-w-4xl rounded-[5rem] p-10 lg:p-16 shadow-2xl relative overflow-y-auto max-h-[95vh] border-4 border-white/10">
                            {!quizState.showResult || quizState.reviewMode ? (
                                <React.Fragment>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            {!quizState.reviewMode ? (
                                                ...
                                                <div className="text-rose-500 font-black uppercase text-[11px] flex items-center gap-2">
                                                    <span className="w-2 h-2 bg-rose-500 rounded-full animate-ping"></span> Chế độ xem lại bài đã làm
                                                </div>
                                            )}
                                        </div>
                                        <button onClick={() => { if(quizState.reviewMode || confirm("Bạn muốn thoát bài làm?")) { setActiveQuiz(null); setQuizState({currentQ:0, answers:[], showResult:false, reviewMode:false}); } }} 
                                    ...
                                    <div className="min-h-[120px] mb-12">
                                        <h3 className="text-2xl lg:text-3xl font-bold text-slate-800 leading-tight">{activeQuiz[quizState.currentQ].q}</h3>
                                    </div>
                                        ...
                                        {activeQuiz[quizState.currentQ].a.map((ans, i) => {
                                            ...
                                            return (
                                                <button key={i} onClick={() => !quizState.reviewMode && handleSelect(i)} 
                                                    ...
                                                    {quizState.reviewMode && isCorrect && <span className="ml-auto text-2xl">⭐</span>}
                                                </button>
                                            );
                                        })}
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-4 pt-8 border-t border-slate-100">
                                        ...
                                        {!quizState.reviewMode && (
                                            <button onClick={() => confirm("Xác nhận nộp bài kiểm tra?") && handleFinish()} className="px-12 py-5 bg-blue-600 text-white rounded-[1.8rem] font-black uppercase text-[10px] shadow-2xl shadow-blue-200 flex-[2]">Nộp bài & Kết thúc</button>
                                        )}
                                    </div>
                                </React.Fragment>
                            ) : (
                                <div className="text-center py-10 animate-in fade-in zoom-in duration-500">
                                    ...
                                    <p className="text-slate-400 font-bold mb-12 text-xl tracking-widest uppercase">Bạn đã hoàn thành tốt bài làm!</p>
                                    ...
                                    <div className="inline-flex flex-col items-center bg-blue-50 p-12 rounded-[4rem] border-4 border-white shadow-inner mb-16">
                                        ...
                                        <div className="text-blue-400 font-black uppercase text-xs mt-4 tracking-[0.5em]">Điểm số / 10</div>
                                    </div>

                                    <div className="flex gap-6 max-w-lg mx-auto">
                                        <button onClick={() => setQuizState({...quizState, reviewMode: true, currentQ: 0})} className="flex-1 bg-slate-100 text-slate-700 py-7 rounded-[2.5rem] font-black uppercase tracking-widest text-[11px] hover:bg-slate-200 transition-all">Xem lại bài làm</button>
                                        <button onClick={() => setActiveQuiz(null)} className="flex-1 bg-slate-900 text-white py-7 rounded-[2.5rem] font-black uppercase tracking-widest text-[11px] shadow-2xl hover:bg-blue-600 transition-all">Đóng cửa sổ</button>
                                    </div>
                                </div>
                            )}
                        </div>
