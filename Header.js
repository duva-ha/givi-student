function Header({ grade, setGrade, user, isFocus, setIsFocus }) {
    // Thêm dòng này để phòng lỗi nếu setIsFocus không được truyền vào
    const toggleFocus = () => {
        if (typeof setIsFocus === 'function') {
            setIsFocus(!isFocus);
        }
    };
    ...
    return (
        <header className="h-16 px-8 border-b bg-white/50 backdrop-blur-xl flex items-center justify-between z-10">
            ...
            {/* Thay onClick trực tiếp bằng hàm toggleFocus an toàn hơn */}
            <button onClick={toggleFocus} className="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 transition-colors shadow-inner">
                {isFocus ? '✕' : '⛶'}
            </button>
        </header>
    );
